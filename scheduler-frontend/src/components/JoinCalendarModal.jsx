...
export default function JoinCalendarModal({ onClose, onJoin }) {
  ...
  const colors = [
    '#FF8A80', '#F8BBD0', '#E53935', '#B71C1C', // reds
    '#D81B60', '#880E4F', // pinks
    '#FFD180', '#E65100', // oranges
    '#FFF59D', '#FFEB3B', '#F9A825', // yellows
    '#A5D6A7', '#43A047', '#1B5E20', // greens
    '#B2DFDB', '#009688', '#004D40', // teals
    '#90CAF9', '#1E88E5', '#0D47A1', // blues
    '#CE93D8', '#8E24AA', '#4A148C', // purples
    '#BCAAA4', '#8D6E63', '#3E2723'  // browns
  ];

  const usedColors = new Set(calendar?.members?.map(m => m.color).filter(Boolean));
  ...
  const handleJoin = async (e) => {
    ...
    try {
      ...
      setErrors({ general: err.response?.data?.error || 'Failed to join calendar.' });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <form
        onSubmit={calendar ? handleJoin : handleCodeSubmit}
      ...
      >
        ...
        {!calendar ? (
          <>
            ...
            />
            {errors.general && <p className="text-red-500 mt-1">{errors.general}</p>}
          </>
        ) : (
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {colors.map(color => (
                <button
                  ...
                  style={{ backgroundColor: color, opacity: usedColors.has(color) ? 0.4 : 1 }}
                />
              ))}
            </div>
